...
//-------------------------------------------------------------
 ...
 typedef struct { // cell properties
				  ...
				  int poreRadNm,bodyFilletNm,vesNum,vesRadNm,tubeGapNm,tubeZnm,raftNum,int8,int9,int10;
...
__published:	// IDE-managed Components
	...
	void __fastcall FuseTubesClick(TObject *Sender);
...
//-------------------------------------------
 ...
 void  Bleaching(void);
 void __fastcall BleachObj(Obj *obj,int num);
 void EMCCD(void);
 ...
 void Fuse(int ind);
 void TubeFusion(int ind,int action);
 bool FindFuseVoxel(int ind);
 void TubeFission(int rad,int cXvox,int cYvox,int cZvox,float stage);
 void MoveToVesicle(Obj *obj, int num,int ind,int medium,int nX,int nY,int nZ);
 ...
 byte *bmpBuf;
 int *layer[3];
 WORD cell[4050][4050],intCCD[4050][4050],*greenCam,*redCam; // cell elements
 AnsiString note;
 ...
 bool outsideVes,mitochondria,tube3D,recycling,fuseTubes;
 int cellXnm,cellYnm,cellZnm,tubeRadNm,sizeXpix,sizeYpix,sizeZpix;
 int cellXvox,cellYvox,cellZvox,shrinkLen;
 int sideGapVox,botGapVox,topGapVox,botGapNm,sideGapNm,topGapNm,tubeLenVox;
 int interTubeDistNm,vesRadNm,bodyFilletNm,vesNum,raftRadNm,tubeStage;
 int tubeLengthNm,matrixXnm,matrixYnm,matrixZnm,tubeGapNm,poreRadNm,poreRadVox;
 int tubeGapVox,tubeRadVox,tubeZnm,tubeZminVox,tubeZmaxVox,raftNum,tubeLeftVox,tubeRightVox;
 ...
 int framesToSum,numOfDarkFrames,ZscanDir,scanCount,savedFrames;
 ...
 double cytoAstatAbindRate,cytoAstatAdissRate,cytoAstepSizeNm,cytoAstepRate,cytoArecycleRate;
 ...
 double membAmob,membApoliRate,membAdepoliRate,membArecycleRate;
