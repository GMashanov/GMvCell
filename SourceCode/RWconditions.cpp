...
//--------------------------------------------------------------------
void TMainForm::ReadSettings(bool action)
{ FILE *infile;
  if ( action )
	{ if ( !OpenPrmDlg->Execute() ) return; }
  else { OpenPrmDlg->FileName="GMvCell.gms"; }
  infile=fopen(AnsiString(OpenPrmDlg->FileName).c_str(),"rt");
  if ( !infile )
	{ fclose(infile); ShowMessage("Can NOT Open Settings (gms) File"); return; }
  ...
  fclose(infile); VarsToPrms(); FormResize(0);
  Canvas->Font->Size=20; Canvas->TextOut(20,20,"Update Matrix if Geometry Changed !!!     ");
}
//---------------------------------------------------------------------------
...
{ int strLength=str.Length(),firstChar=0,comLength=0,index=0;
 ...
 // cell property
  ...
  if ( param == "raftNumber" )  { raftNum=val.ToIntDef(10); return; }
  ...
  if ( param == "interTubeDistNm" ) { interTubeDistNm=val.ToIntDef(1000); return; }
  if ( param == "tubeZnm" ) { tubeZnm=val.ToIntDef(200); return; }
  ...
  if ( param == "vesicleRadNm" )  { vesRadNm=val.ToIntDef(100); return; }
  ...
  if ( param == "greenBlinking" )  { greenBlinking=state; return; }
  if ( param == "greenBlinkONrate" ) { greenBlinkONrate=StrToFloatDef(val,1.0); return; }
  if ( param == "greenBlinkOFFrate" ) { greenBlinkOFFrate=StrToFloatDef(val,1.0); return; }
  ...
  if ( param == "statAdistribution" )
	{ if ( val == "STRAIT_FIBERS_XY" ) { statAinit=0; return; }
	  ...
	  if ( val == "3D_FIBERS_CENTROSOME" ) { statAinit=5; return; }
	  if ( val == "SPECIAL" ) { statAinit=6; return; }
	  if ( val == "SPECIAL2" ) { statAinit=7; return; }
	}
...
{ FILE *outfile;
  ...
  str+="raftNumber,"; str+=raftNum; str+=";\n";
 ...
 //-------------   Illumination
  ...
  str+="greenBlinking,";  if ( greenBlinking ) str+="ON"; else str+="OFF"; str+=";\n";
  str+="greenBlinkONrate,"; str+=Round(greenBlinkONrate,5); str+=";\n";
  str+="greenBlinkOFFrate,"; str+=Round(greenBlinkOFFrate,5); str+=";\n";
  ...
  str+="greenFluorsPerMembA,"; str+=greenMembAnum;  str+=";\n";
  str+="greenFractionMembA%,"; str+=greenMembAfrac;  str+=";\n";
  str+="redFluorsPerMembA,"; str+=redMembAnum;  str+=";\n";
  str+="redFractionMembA%,"; str+=redMembAfrac;  str+=";\n";
  str+="membAdiffusionRate,"; str+=membAmob;  str+=";\n";
  ...
  if ( statAinit == 5 ) str+="3D_FIBERS_CENTROSOME;\n";
  if ( statAinit == 6 ) str+="SPECIAL;\n";
  if ( statAinit == 7 ) str+="SPECIAL2;\n";
