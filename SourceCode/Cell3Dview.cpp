	...
	{ for ( int i=0; i < MainForm->membAnum; i++ )
		 { y=MainForm->MembA[i].y/nmVox;
		  ...
		  // if ( mat[x+(y*matrixX)+(z*matrixXY)] < 200 ) continue; // no memb
		   mat[x+(y*matrixX)+(z*matrixXY)]=MainForm->MembA[i].medium-5;  // main cell membrane
		 }
	   for ( int i=0; i < MainForm->membBnum; i++ )
		 { y=MainForm->MembB[i].y/nmVox;
		   ...
		   if ( mat[x+(y*matrixX)+(z*matrixXY)] < 200 ) continue;  // no memb
		   mat[x+(y*matrixX)+(z*matrixXY)]=CELL_MEMB-4;  // main cell membrane
		   if ( MainForm->membBinit == 2 )
			 mat[x+(y*matrixX)+(z*matrixXY)]=NUCL_MEMB-4; //nucleus membr
		   if ( MainForm->membBinit == 3 )
			 mat[x+(y*matrixX)+(z*matrixXY)]=TUBE_MEMB-4; // tube membr
		   if ( MainForm->membBinit == 4 )
			 mat[x+(y*matrixX)+(z*matrixXY)]=VESI_MEMB-4; // vesicle membr
		}
	 }
   if ( MainForm->ShowCytoObj->Checked )
	 for ( int i=0; i < MainForm->cytoAnum; i++ )
		{ y=MainForm->CytoA[i].y/nmVox;
		 ...
		 // if ( mat[x+(y*matrixX)+(z*matrixXY)] ==  || mat[x+(y*matrixX)+(z*matrixXY)] == 50 )
			mat[x+(y*matrixX)+(z*matrixXY)]=10;
		}
}
//---------------------------------------------------------------------------
		...
		{ color=matrix[x+(y*matrixX)+(z*matrixXY)];
		  ...
		  if ( color == STAT_VOXEL )  // Static Object
			{ MainForm->matrix[x+(y*matrixX)+(z*matrixXY)]=CYTOPLASM;
			 ...
			 // DrawObj((x+y*swing)*zoom,oriY+(y*tilt-z)*zoom*tilt,40);
			  DrawObj(oriX+(x+y*swing)*zoom,oriY+(y*tilt-z)*zoom*tilt,40);
			  continue;
			}
		  if ( color == 10 )  // Cyto1 Object
			{ matrix[x+(y*matrixX)+(z*matrixXY)]=CYTOPLASM; // return to empty voxel
			  DrawObj((x+y*swing)*zoom,oriY+(y*tilt-z)*zoom*tilt,10);
			  continue;
			}
		  if ( color == CELL_MEMB-5 || color == NUCL_MEMB-5 ||
			   color == TUBE_MEMB-5 || color == VESI_MEMB-5 )  // Memb1
			{ matrix[x+(y*matrixX)+(z*matrixXY)]=color+5; // restore value
			  DrawObj((x+y*swing)*zoom,oriY+(y*tilt-z)*zoom*tilt,20);
			  continue;
			}
		  if ( color == CELL_MEMB-4 || color == NUCL_MEMB-4 ||
			   color == TUBE_MEMB-4 || color == VESI_MEMB-4 )  // Memb2
			{ matrix[x+(y*matrixX)+(z*matrixXY)]=color+4; // restore value
