...
{ LengthEdit->Text=cond.cellXnm*0.001; HeightEdit->Text=cond.cellZnm*0.001;
  ...
  NucleusRadEdit->Text=cond.nucleusRadNm;   VelocityEdit->Text=cond.velocity;
  ...
  FuseVesiclesChBox->Checked=cond.fuseVes;
  ...
  AAdissRateEdit->Text=cond.membAAdissRate; AAbindingRateEdit->Text=cond.membAAbindRate;
  MembAinitBox->ItemIndex=cond.membAinit; MembApoliNumEdit->Text=cond.membApoliNum;
  CytoAcanBindMembA_ChBox->Checked=cond.cytoAcanBindMembA;
  MembApolimerChBox->Checked=cond.membApolimer;
  MoveToNucleusChBox->Checked=cond.membAnuclTubePump;
 // Memb B prop
  ...
  GreenStatAfractionEdit->Text=cond.greenStatAfraction;
  RedStatAnumEdit->Text=cond.redStatAnum;
  RedStatAfractionEdit->Text=cond.redStatAfraction;
  ...
  VelocityEdit->Enabled=cond.cytoAcanMoveAlongStatA;
  ...
  IllumFWHMEdit->Text=cond.illuminationFWHM*0.001;
  TimeIntervalEdit->Text=cond.tmInterMs; MonomersInFilamentEdit->Text=cond.monomersPerFilament;
  IrisRadiusEdit->Text=cond.irisRadiusNm*0.001;
  RaftMobRatioEdit->Text=cond.raftMobRatio;
...
{ int buf=StrToFloatDef(LengthEdit->Text,-1.0)*1000.0;
  ...
  LengthEdit->Text=buf*0.001; cond.cellXnm=buf; Statistics();
}
//---------------------------------------------------------------------------
...
{ cond.rafts=RaftsChBox->Checked;  DrawCell();
  RaftSizeEdit->Enabled=cond.rafts;
...
{ int buf=StrToIntDef(CytoAnumEdit->Text,-1);
  if ( buf < 0 || buf > CYTO_NUM )
    {ShowMessage("Wrong number of CytoA molecules");CytoAnumEdit->Text=cond.cytoAnum;return; }
...
{ int buf=StrToIntDef(MembAnumEdit->Text,-1);
  if ( buf < 0 || buf > MEMB_NUM )
    { ShowMessage("Wrong number of memb A objects");MembAnumEdit->Text=cond.membAnum;return; }
...
{ cond.membApolimer=MembApolimerChBox->Checked; }
//---------------------------------------------------------------------------
...
{ double buf=StrToFloatDef(AAdissRateEdit->Text,-1.0);
  if ( buf <= 0.0 || buf > 10000 )
    { ShowMessage("AA dissociation rate must be 0-10000 s-1");
      AAdissRateEdit->Text=cond.membAAdissRate; return;
    }
  cond.membAAdissRate=buf;
}
//---------------------------------------------------------------------------
...
{ int buf=StrToIntDef(MembBnumEdit->Text,-1);
  if ( buf < 0 || buf > MEMB_NUM )
    { ShowMessage("Wrong number of site B molecules"); MembBnumEdit->Text=cond.membBnum;return; }
...
{ double buf=StrToFloatDef(IllumFWHMEdit->Text,-1.0)*1000.0;
  if ( buf < 100 || buf > 1000000 )
    { ShowMessage("Illumination Profile is out of Order (0.1-1000)");
      IllumFWHMEdit->Text=cond.illuminationFWHM*0.001;return;
    }
  cond.illuminationFWHM=buf; IllumFWHMEdit->Text=cond.illuminationFWHM*0.001; DrawGauss();
}
//---------------------------------------------------------------------------
...
{ int left=30,top=30; float zoom=1.0;
  float ratio=((float)(cond.illuminationFWHM)/(float)(cond.cellXnm))*200;
  double sigma=ratio/2.3548;  float irisRad=((cond.irisRadiusNm)/(float)(cond.cellXnm))*100;
  GaussImage->Canvas->Brush->Color=clBtnFace;
...
{ double buf=StrToFloatDef(IrisRadiusEdit->Text,-1.0)*1000.0;
  if ( buf < 100 || buf > 2000000 )
    { ShowMessage("Diaphragm Diameter is out of Order (0.1-2000)");
      IrisRadiusEdit->Text=cond.irisRadiusNm*0.001;return;
    }
  cond.irisRadiusNm=buf; IrisRadiusEdit->Text=cond.irisRadiusNm*0.001; DrawGauss();
}
//---------------------------------------------------------------------------
...
{ double buf=StrToFloatDef(GainEdit->Text,-1);
  if ( buf < 0.0 || buf > 50 )
	{ ShowMessage("Wrong Gain Level (0-50)"); GainEdit->Text=cond.gain; return; }
  cond.gain=buf;
}
//---------------------------------------------------------------------------
...
{ double buf=StrToFloatDef(AAbindingRateEdit->Text,-1);
  if ( buf < 1 || buf > 1e7 )
    { ShowMessage("Rate is out of Range (1-1e7 s-1)");
      AAbindingRateEdit->Text=cond.membAAbindRate; return;
    }
  cond.membAAbindRate=buf;
}
//---------------------------------------------------------------------------
...
{ int buf=StrToIntDef(StatAnumEdit->Text,-1);
  if ( buf < 0 || buf > STAT_NUM )
    { ShowMessage("Wrong number of static A objects");StatAnumEdit->Text=cond.statAnum;return; }
...
{ int buf=StrToIntDef(GreenStatAnumEdit->Text,-1);
  if ( buf < 0 || buf > 4 )
    { ShowMessage("Wrong number of Fluorophores per Object");
...
{ int buf=StrToIntDef(GreenStatAfractionEdit->Text,-1);
  if ( buf < 0 || buf > 100 )
    { ShowMessage("Wrong number of Fluorophores per Object");
      GreenStatAfractionEdit->Text=cond.greenStatAfraction;return;
    }
  cond.greenStatAfraction=buf; Statistics();
}
//---------------------------------------------------------------------------
...
{ int buf=StrToIntDef(RedStatAfractionEdit->Text,-1);
  if ( buf < 0 || buf > 100 )
    { ShowMessage("Wrong number of Fluorophores per Object");
      RedStatAfractionEdit->Text=cond.redStatAfraction; return;
    }
  cond.redStatAfraction=buf; Statistics();
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void __fastcall TSForm::VelocityEditExit(TObject *Sender)
{ double buf=StrToFloatDef(VelocityEdit->Text,-1.0);
  if ( buf < 0.001 || buf > 50.0 )
    { ShowMessage("Velocity range (0.001 and 50) um/s");
      VelocityEdit->Text=cond.velocity; return;
    }
  cond.velocity=buf;
}
//---------------------------------------------------------------------------
void __fastcall TSForm::VelocityEditKeyDown(TObject *Sender, WORD &Key,
      TShiftState Shift)
{ if ( Key == 13 ) FlatteningEditExit(Sender); }
//---------------------------------------------------------------------------
...
{ int buf=StrToIntDef(CytoBnumEdit->Text,-1);
  if ( buf < 0 || buf > CYTO_NUM )
    { ShowMessage("Wrong number of CytoB molecules");
...
{ int buf=StrToIntDef(MembCnumEdit->Text,-1);
  if ( buf < 0 || buf > MEMB_NUM )
    { ShowMessage("Wrong number of MembC molecules");
...
{ double buf=StrToFloatDef(CytoBmobEdit->Text,-1.0);
  if ( buf < 0.0 || buf > 50.0 )
	{ ShowMessage("Wrong Value of CytoB Diffusion Rate");CytoBmobEdit->Text=cond.cytoBmob;return; }
...
{ cond.movingVesicles=MovingVesChBox->Checked; VesRadEdit->Enabled=MovingVesChBox->Checked;
  VesNumEdit->Enabled=MovingVesChBox->Checked; DrawCell();
}
//---------------------------------------------------------------------------
...
{ cond.membAnuclTubePump=MoveToNucleusChBox->Checked; }
//---------------------------------------------------------------------------
