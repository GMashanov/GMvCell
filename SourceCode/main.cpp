...
{ struct  time t;        Left=10;Top=5;
  gettime(&t); srand(43);  //GM
  for ( int i=0; i < t.ti_min+t.ti_sec; i++ ) random(t.ti_min+t.ti_sec);  //GM
  prm.leftX=prm.topY=0; prm.Igain=prm.Vgain=prm.bin=1;
  nmVox=5; nm_pix=5; memorySizeMb=MATRIX_SIZE/(1e6);
  // cell properties
  membOK=false;  makePore=true; movingVes=false;  roundedCellBody=true;
  ...
  numOfDarkFrames=0;  fenceJumpProb=20; raftMobRatio=0.1; nucleusRadNm=400;
 ...
 // twoCam=true;  // tube3D=true;
  ...
  velocity=5.0; /*um/s */   tmStep=0.005; // (s)
  // illumination and camera prop
  Kobj=0.6; EMCCDmode=true; noiseAmp=0.5; gain=5; framesToSum=10;
  FWHM=250; greenPhotonsPerSec=6000,redPhotonsPerSec=10000;
  illuminationFWHM=100000.0; bleachSize=5000; TIRangle=63;
  ...
  irisRadiusNm=50000;
  TrackBar->Position=focusPos=0; ZscanDir=1;
 // ShowMessage(d561);
 ...
 // Cyto B prop
  ...
  membAinit=1;membAAdissRate=1.0;monomersPerFilament=500;membAAbindRate=100000.0;
  ...
  greenStatAfraction=100,greenStatAnum=2,redStatAfraction=100,redStatAnum=0;
...
{ matrixX=cellXvox=cellXnm/nmVox; matrixY=cellYvox=cellYnm/nmVox;
  ...
  sizeXpix=matrixXnm/nm_pix; sizeYpix=matrixYnm/nm_pix;sizeZpix=matrixZnm/nm_pix;
		...
		{ MembA[i].x=sideGapNm+10; MembA[i].y=sideGapNm+10;
		  for ( int j=0; j < 1000; j++ )
			{ xx=random(cellXnm-10)+sideGapNm; yy=random(cellYnm-10)+sideGapNm;
			  if ( maskBmp->Canvas->Pixels[xx/nm_pix][yy/nm_pix] > 0 )
  ...
  for ( int i=0; i < statAnum; i++ ) // set binding and fluorescence
	{ StatA[i].cytoAind=StatA[i].cytoBind=-1;StatA[i].membAind=StatA[i].membBind=-1;
	  ...
	  for ( int j=0; j < greenStatAnum; j++ )
		if ( random(100) < greenStatAfraction ) StatA[i].green[j]=true;
	  for ( int j=0; j < redStatAnum; j++ )
		if ( random(100) < redStatAfraction ) StatA[i].red[j]=true;
...
{ float jump=JumpSizeNm(tmStep,cytoAmob),nX,nY,nZ,dX,dY,dZ,rXY,tX,tY,tZ,prob;
  ...
  for ( int i=0,ind; i < cytoAnum; i++ )
	{ if ( CytoA[i].medium == NUCLEOPLASM || CytoA[i].medium == T_LUMEN )
		cytoAinNucleus++;
...
{ float dX,dY,dZ,prob;  int medium=CYTOPLASM;
  double dissRateA=(1.0/(cytoAmembAdissRate*tmStep)),dissRateAA=(1.0/(membAAdissRate*tmStep));
  if ( membApolimer ) // polimerisation
		...
		{ if ( MembA[i].siteAind > -1 ) continue; // skip if membA already bound
		  for ( int j=0; j < membAnum; j++ ) // check for possible binding
			{ if ( MembA[j].siteBind > -1 || i == j ) continue; // skip if site already bound
			  ...
			  prob=timeDistMembAA[dist]; prob*=membAAbindRate;
  ...
  for ( int i=0; i < membAnum; i++ ) // depolimerization
	  { if ( MembA[i].siteBind < 0  ) continue; // skip if site inside polimer
		if ( random(dissRateAA+1.0) < 1 ) // dissociation
	...
	if ( (j=MembA[i].cytoAind) > -1 ) // possible CytoA dissociation
	 if ( random(dissRateA+1.0) < 1 )
	   { medium=CytoA[j].medium;
		 if ( membAnuclTubePump ) // change CytoA medium type to where it pumped
		   if ( MembA[i].medium == NUCL_MEMB ) medium=NUCLEOPLASM;
		   else                                medium=T_LUMEN;
		 if ( MoveToNearestVoxel(&CytoA[j],10,medium) ) // move to nucleus voxel
		   { if ( membAnuclTubePump )
		       { CytoA[j].medium=medium;  } // restore fluorescence if needed
	...
	{ if ( MembA[i].cytoAind > -1 ) continue; // skip if membA occupied
	  for ( int j=0,dist; j < cytoAnum; j++ )
		{ if ( CytoA[j].membAind > -1 || CytoA[j].membBind > -1 ) continue; // already bound
		  ...
		  if ( membAnuclTubePump && CytoA[j].medium != CYTOPLASM ) continue; // already flipped
...
{ float dX,dY;
  ...
  cytoAboundToCytoBnum=cytoAboundToStatAnum=0,cytoAinNucleus=0;
...
{ int dX=x-(cellXnm*0.5)+sideGapNm,dY=y-(cellYnm*0.5)+sideGapNm,gaussX,gaussY;
  if ( (dX*dX)+(dY*dY) > (irisRadiusNm*irisRadiusNm) ) return 0;
  ...
  else photons*=Gauss2D(dX,dY,illuminationFWHM/2.355);
  photons+=GaussRand()*sqrt(photons);
  if ( photons < 1.0 ) return 0;
  float curFWHM=Kobj*0.001*(focus-z)*(focus-z)+FWHM, sigma=curFWHM/2.355;
  int redShift=0;
	...
	{ if ( blueLaser ) // Green fluorescence
		for ( int j=0; j < 4; j++ )
		  { //if ( obj[i].cytoBind < 0 ) continue; // for calcium imaging only
			...
			outcome=Emission(obj[i].x,obj[i].y,obj[i].z,focusPos,greenPhotons,GREEN);
			if ( outcome > 0 )  // photobleaching
			  if ( random(greenBleachPerFrame/outcome) < 1 ) obj[i].green[j]=false;
			if ( FRETon ) // check red fluorophores around
			  { float dX,dY,dZ,dist;
				  ...
				  { dX=obj[i].x-MembB[k].x; dY=obj[i].y-MembB[k].y;
					...
					for ( int j=0; j < 4; j++ )
					  { if ( !MembB[k].red[j] ) continue;  // not active
						Emission(MembB[k].x,MembB[k].y,MembB[k].z,focusPos,redPhotons/(dist+1.0),RED);
					  }
				  }
			  }
		  }
      if ( greenLaser ) // red fluorescence
		for ( int j=0; j < 4; j++ )
		  { if ( !obj[i].red[j] ) continue;
			outcome=Emission(obj[i].x,obj[i].y,obj[i].z,focusPos,redPhotons,RED);
            if ( outcome > 0 )  // photobleaching
  ...
  if ( ShowStaticObj->Checked ) // Static Objects drawn first
	{ bmp->Canvas->Pen->Color=(TColor)RGB(20,20,254);
	  for ( int i=0,x,y; i < statAnum; i++ )
		{ x=StatA[i].x/nm_pix; y=StatA[i].y/nm_pix;
  ...
  if ( ShowMembObj->Checked )
	 { bmp->Canvas->Pen->Color=(TColor)RGB(250,250,0); bmp->Canvas->Brush->Style=bsClear;
	   for ( int i=0,x,y; i < membAnum; i++ )
		 { x=MembA[i].x/nm_pix; y=MembA[i].y/nm_pix;
		   bmp->Canvas->Rectangle(x-1,y-1,x+1,y+1);
	   ...
	   for ( int i=0,x,y; i < membBnum; i++ )   // mark membB
		 { x=MembB[i].x/nm_pix; y=MembB[i].y/nm_pix;
		   bmp->Canvas->Rectangle(x-1,y-1,x+1,y+1);
	   ...
	   for ( int i=0,x,y; i < membCnum; i++ )
		 { x=MembC[i].x/nm_pix; y=MembC[i].y/nm_pix;
		   bmp->Canvas->Rectangle(x-1,y-1,x+1,y+1);
   ...
   if ( ShowCytoObj->Checked )
	 { bmp->Canvas->Pen->Color=(TColor)RGB(254,254,254);
	   ...
	   for ( int i=0,x,y; i < cytoAnum; i++ )
		 { x=CytoA[i].x/nm_pix; y=CytoA[i].y/nm_pix;
		   bmp->Canvas->Rectangle(x-1,y-1,x+1,y+1);
		 }
	   bmp->Canvas->Pen->Color=(TColor)RGB(160,160,160);
	   for ( int i=0,x,y; i < cytoBnum; i++ )
		 { x=CytoB[i].x/nm_pix; y=CytoB[i].y/nm_pix;
  ...
  while ( abort == false && membOK )     // no running with unprepared membrane
	{ StepCalc();
		...
		{ AnsiString str=bmpName; str+=frameCount; str+=".bmp";
		  if ( Cell3D->Visible )
			...
			{ Bmp24->Canvas->CopyRect(Rect(0,0,Bmp24->Width,Bmp24->Height),
			  ...
			  str+=focusPos; str+="nm ("; str+=nm_pix; str+="nm/pix         ";
	  ...
	  if ( Zscan->Checked && frameCount%10 == 0 )
		{ focusPos+=ZscanDir*100;
		  if ( focusPos > cellZnm+botGapNm ) ZscanDir=-1;
		  if ( focusPos <= 0 ) ZscanDir=1;
		  TrackBar->Position=focusPos; scanCount=0;
		}
	}
...
{ int *ptr,val=0; double scl=(double)nm_pix/(double)nmVox;
...
{ static int indAA=0; double averAA=0.0;
  ...
  AnsiString str=nm_pix; str+=" nm/pix, Focus="; str+=focusPos;
  ...
  if ( membAnuclTubePump )
	{ str+=" ";  str+=cytoAinNucleus; str+=" CytoA in nucleus/ER,"; }
  if ( membAboundMembA )
	{  indAA++;
	   if ( indAA > 99 ) indAA=0;
...
{ // Cell Properties
  ...
  SForm->cond.rafts=rafts; SForm->cond.nucleusProt=nucleusProt;
  ...
  SForm->cond.Kobj=Kobj; SForm->cond.TIRangle=TIRangle; SForm->cond.FWHM=FWHM;
  SForm->cond.illuminationFWHM=illuminationFWHM; SForm->cond.irisRadiusNm=irisRadiusNm;
  SForm->cond.framesToSum=framesToSum;  SForm->cond.nm_pix=nm_pix;
  ...
  SForm->cond.membAAdissRate=membAAdissRate;  SForm->cond.membAAbindRate=membAAbindRate;
  SForm->cond.cytoAmembAdissRate=cytoAmembAdissRate;
  SForm->cond.membAmembBdissRate=membAmembBdissRate;
  SForm->cond.membApolimer=membApolimer;
  ...
  SForm->cond.greenStatAfraction=greenStatAfraction; SForm->cond.redStatAfraction=redStatAfraction;
  ...
  SForm->cond.membAclusterNum=membAclusterNum; SForm->cond.velocity=velocity;
...
{  if ( SForm->cond.memorySizeMb != memorySizeMb )  // resize main memory
  ...
  if ( cellXnm != SForm->cond.cellXnm || cellYnm != SForm->cond.cellYnm ||
	   ...
	   nm_pix != SForm->cond.nm_pix ||  SForm->cond.nucleus != nucleus ||
	   SForm->cond.tubeNetwork != tubeNetwork || SForm->cond.grid != grid ||
	   SForm->cond.rafts != rafts || SForm->cond.nucleusProt != nucleusProt ||
	...
	{ ShowMemb->Checked=false; currentZ=0;// new membrane
	 ...
	 // nm_pix=SForm->cond.nm_pix;
	  ...
	  nucleus=SForm->cond.nucleus; nucleusProt=SForm->cond.nucleusProt;
  ...
  if ( // cyto A prop
	  ...
	  // memb A check
	   ...
	   greenStatAfraction != SForm->cond.greenStatAfraction ||
	   redStatAfraction  != SForm->cond.redStatAfraction ||
  ...
  noiseAmp=SForm->cond.noise;   gain=SForm->cond.gain;  FWHM=SForm->cond.FWHM;
  ...
  irisRadiusNm=SForm->cond.irisRadiusNm; illuminationFWHM=SForm->cond.illuminationFWHM;
  ...
  FRETon=SForm->FRETchBox->Checked; numOfDarkFrames=SForm->cond.numOfDarkFrames;
  ...
  // memb A prop
  membAmob=SForm->cond.membAmob;  membAnuclTubePump=SForm->cond.membAnuclTubePump;
  ...
  membAAdissRate=SForm->cond.membAAdissRate;membAAbindRate=SForm->cond.membAAbindRate;
  membApolimer=SForm->cond.membApolimer;
  ...
  // stat A prop
  statAinit=SForm->cond.statAinit; velocity=SForm->cond.velocity;
...
{ if ( nm_pix < 10 ) return;
  ...
  if ( CCDwidth > 2040 || CCDheight > 2040 )
	{ nm_pix+=5; ShowMessage("Pixel size is too small for chosen cell sizes"); }
  FormResize(Sender);
}
//---------------------------------------------------------------------------
...
{ int max=0,szX=matrixXnm/nm_pix,szY=(matrixYnm)/nm_pix;
  ...
  prm.X_nm_pix=prm.Y_nm_pix=nm_pix; prm.params&=128; // varible
  prm.Igain=gain; prm.addInfo=1;  addPrm.stageXnm=0; addPrm.stageYnm=0;
  addPrm.focusZnm=focusPos;   addPrm.TIRangle=TIRangle;
...
{ TrackBar->PageSize=cellZnm/10; focusPos=TrackBar->Position; FormPaint(Sender); }
//---------------------------------------------------------------------------
void __fastcall TMainForm::Bleaching(void)
{ int centX=cellXnm/2+sideGapNm,centY=sideGapNm+cellYnm/2,dX,dY;
  int boader=sideGapNm+cellXnm-bleachSize,rad=bleachSize/2;
...
{ double size=((double)bleachSize)*0.001; UnicodeString str=size;
