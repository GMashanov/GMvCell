...
//
//  GMvCell project, G. Mashanov, Crick Institute,2015-2024
// development of GMcellModel, 2007-2014
//---------------------------------------------------------------------------
#include <vcl.h>
#pragma hdrstop
#include <math.h>
...
{ struct  time t;        Left=10;Top=5;
 ...
 // General properties
  ...
  cellXnm=5000; cellYnm=5000; cellZnm=600; tubeRadNm=100; bodyFilletNm=300;
  sideGapNm=200;botGapNm=40;topGapNm=40;//filopodia=true;tubeNetwork=true;nucleus=true;
  vesNum=3; interTubeDistNm=1000; tubeLengthNm=3000; tubeGapNm=300; vesRadNm=100; tubeZnm=180;
  fenceJumpProb=20; raftMobRatio=0.1; nucleusRadNm=400; raftNum=100;
  ...
  vesicleMob=0.02; fuseVesicles=false; recycling=false;
 // twoCam=true;  // tube3D=true;
  CalculateDimensions();
  bendTilt=1.0; // vesicles horisontal
  // illumination and camera prop
   numOfDarkFrames=0;
  Kobj=0.6; EMCCDmode=true; noiseAmp=1.5; gain=5; framesToSum=10;
 ...
 // ShowMessage(d561);
  greenBleachRate=0.05; redBleachRate=0.02; // (s-1)
  ...
  cytoApoliRate=5e5;  cytoAdepoliRate=0.1;  cytoArecycleRate=0.1;
 ...
 // Cyto B prop
  ...
  redMembAfrac=100; membApoliNum=8; membAclusterNum=8; membArecycleRate=0.1;
  // Memb B prop
  ...
  greenCam=new WORD[camX*camX];  redCam=new WORD[camX*camX];
...
{ matrixX=cellXvox=cellXnm/nmVox; matrixY=cellYvox=cellYnm/nmVox;
  ...
  tubeLeftVox=matrixX*0.5-(tubeLenVox*0.5);tubeRightVox=matrixX*0.5+(tubeLenVox*0.5);
...
{ int dX,dY,dZ,rrr;
  int beg=cXvox-rad,end=cXvox+tubeLenVox+rad,mid=cXvox+tubeLenVox*0.5;
	...
	for ( __int64 y=cYvox-rad-2; y < cYvox+rad+2; y++ )
	  for ( __int64 z=cZvox-rad-2; z < cZvox+rad+2; z++ )
		{ dY=cYvox-y; dZ=cZvox-z;
		  ...
		  if ( x > beg+rad && x < mid )  dX=((beg+rad)-x)*stage;
		  if ( x < end-rad && x >= mid ) dX=((end-rad)-x)*stage;
		  rrr=sqrt((dY*dY)+(dZ*dZ)+(dX*dX));
		  if ( rrr < rad+1 ) matrix[x+(y*matrixX)+(z*matrixXY)]=CYTOPLASM; // replace old structure
		  if ( rrr < rad )   matrix[x+(y*matrixX)+(z*matrixXY)]=VESI_MEMB;
		  if ( rrr < rad-3 ) matrix[x+(y*matrixX)+(z*matrixXY)]=V_LUMEN;
	   }
}
//----------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void TMainForm::MoveToVesicle(Obj *obj, int num,int ind,int medium,int nX,int nY,int nZ)
{ int cXnm=vesicle[ind].iniXvox*nmVox,cYnm=vesicle[ind].iniYvox*nmVox;
  int cZnm=vesicle[ind].iniZvox*nmVox,dX,dY,dZ;
  for ( int i=0; i < num; i++ ) // move MembA from tube to vesicle
	if ( obj[i].vesicleInd < 0 )
	  { dX=obj[i].x-cXnm;dY=obj[i].y-cYnm; dZ=obj[i].z-cZnm;
		if ( sqrt(dX*dX+dY*dY+dZ*dZ) < tubeRadNm+nmVox )
		  { obj[i].vesicleInd=ind; obj[i].medium=medium;
			obj[i].x=nX*nmVox;obj[i].y=nY*nmVox;obj[i].z=nZ*nmVox; // vesicle center
			MoveToNearestVoxel(&obj[i],vesRadNm/nmVox+2,obj[i].medium);
		  }
	   }
}
//---------------------------------------------------------------------------
void TMainForm::GenerateVesicle(int ind)
{ if ( !vesicle[ind].fused ) return; // not ready for recycling
  ...
  MoveToVesicle(CytoA,cytoAnum,ind,V_LUMEN,nX,nY,nZ);
  MoveToVesicle(MembA,membAnum,ind,VESI_MEMB,nX,nY,nZ);
  ...
  vesicle[ind].docked=vesicle[ind].fused=false;
}
	...
	for ( int y=-rad-1; y <= rad+1; y++ )
	  for ( int z=-rad-1; z <= rad+1; z++ )
		{ absX=cX+x;absY=cY+y;absZ=cZ+z; dist=sqrt((x*x)+(y*y)+(z*z));
		  ...
		  if ( dist <= rad && dist > rad-2 && val != noGo && val != lumen )
...
//----------------------------------------------------------------------------
void TMainForm::TubeFusion(int ind,int action) // 1 - make pore, 2 - shink, 3 - collapce
{ float cX=vesicle[ind].xVox,cY=vesicle[ind].yVox,cZ=vesicle[ind].zVox;
  int rad=vesicle[ind].radNm/nmVox,poreR=poreRadVox,delMed=CYTOPLASM;
  int len=rad+poreR*2,voxVal,medium=VESI_MEMB,noGo=EXTRACELL,cytoMed=V_LUMEN;
   __int64 absX,absY,absZ;
  if ( action == 0 ) // make pore
	for ( int i=cZ; i > 0; i-- )
	  FillSphere(poreR,cX,cY,i,CELL_MEMB,noGo,noGo);
  if ( action == 1 ) // make pore
	{ FillSphere(rad,cX+shrinkLen,cY,cZ+(float)shrinkLen*bendTilt,1,1,1);
	  for ( int x=tubeLeftVox,z; x < tubeLeftVox+shrinkLen; x++ )
		{ z=(x-tubeLeftVox)*bendTilt;
		  FillSphere(rad,x,cY,z+cZ,CELL_MEMB,noGo,noGo);
		}
	  for ( int i=cZ; i > 0; i-- )  // remake pore
		FillSphere(poreR,cX,cY,i,CELL_MEMB,noGo,noGo);
	}
  if ( action == 2 )
	for ( int z=cZ+rad+2,med=1; z > 0; z-- )
	  { absX=-rad+cX-2; absY=-rad+cY-2;absZ=z;
		med=matrix[absX+(absY*matrixX)+(absZ*matrixXY)];
		for ( int y=-rad-1; y < rad+1; y++ )
		  for ( int x=-rad-1; x < rad+1; x++ )
			{ absX=x+cX;absY=y+cY;matrix[absX+(absY*matrixX)+(absZ*matrixXY)]=med; }
	  }
}
//----------------------------------------------------------------------------
...
{ float cX=vesicle[ind].xVox,cY=vesicle[ind].yVox,cZ=vesicle[ind].zVox;
 ...
 //  if ( ind == 3 ) len=0;
  ...
  for ( int i=0; i < cytoAnum; i++ ) // change medium type and remove vesicle Index
  ...
  for ( int i=0; i < membBnum; i++ )   // move membB out of the pore
	if ( MembB[i].vesicleInd == ind )
	  { MoveToNearestVoxel(&MembB[i],len,medium); MembB[i].vesicleInd=-1; }
  for ( int i=0; i < membCnum; i++ )   // move membC out of the pore
	if ( MembC[i].vesicleInd == ind )
	  { MoveToNearestVoxel(&MembC[i],len,medium); MembC[i].vesicleInd=-1; }
  vesicle[ind].fused=true;
  if ( transfer ) FillSphere(rad,cX,cY,cZ,delMed,delMed,delMed);// remove vesicle
}
//----------------------------------------------------------------------------
void TMainForm::MoveVesicle(int ind)
{ __int64 absX,absY,absZ; bool freeToGo=true;
  if ( vesicle[ind].fused ) return;  // still not in use
 ...
 ///// if ( vesicle[ind].z > botGapNm+200 ) return; // for Fig 5 only
 ////  if ( vesicle[ind].y < matrixYnm/2-20 || vesicle[ind].y > matrixYnm/2+20 ) return; // for YouTube

  if ( fuseVesicles && !freeToGo )  // if fusion is ON and vesicle is close to cell memb
  ...
  for ( __int64 y=yO+1; y < yO+rad; y++ ) // make a gap
	for ( __int64 z=zO; z < zO+rad*2; z++ )
	  for ( __int64 x=xO+1; x < xO+gap-1; x++ )
		if ( matrix[x+(y*matrixX)+(z*matrixXY)] != VESI_MEMB &&
		     matrix[x+(y*matrixX)+(z*matrixXY)] != CYTOPLASM )
		matrix[x+(y*matrixX)+(z*matrixXY)]=V_LUMEN;
}
//--------------- units in voxels --------- for mitochondria septas -----------
...
{ double azi,ele,tubeX,tubeY,tubeZ,newZ,m1,m2; float dAzi,dEle,step=(float)rad*0.2;
  double dX1=0.0,dX2=0.0,dY1=0.0,dY2=0.0,dZ1=0.0,dZ2=0.0,avCos=0.0,avLen=0.0,pL=0.0;
  for ( int i=0,medium; i < vesNum; i++ )
	{ tubeZ=tubeZnm/nmVox; azi=random(360);ele=0.0; dX2=dY2=dZ2=0.0;
	  for ( int att=0; att < 210; att++ )
		{ tubeX=random(matrixX); tubeY=random(matrixY);
		  if ( CheckSphere(rad+2,tubeX,tubeY,tubeZ,T_LUMEN,TUBE_MEMB,CYTOPLASM) )
			{ vesicle[i].iniXvox=tubeX;vesicle[i].iniYvox=tubeY;
			  vesicle[i].iniZvox=tubeZ; vesicle[i].fused=true; break;
			}
		  if ( att > 200 ) { ShowMessage("No space to start the tube"); return; }
	  ...
	  while( CheckSphere(rad+3,tubeX,tubeY,tubeZ,T_LUMEN,TUBE_MEMB,CYTOPLASM) )
		{ FillSphere(rad,tubeX,tubeY,tubeZ,TUBE_MEMB,T_LUMEN,CELL_MEMB);
		  dX1=dX2;dY1=dY2; tubeX+=dX2=COS(azi)*COS(ele)*step;
		  tubeY+=dY2=SIN(azi)*COS(ele)*step; azi+=RandOne()*bendTilt;
		  if (tube3D )  // meandring in Z
			{ ele+=RandOne()*bendTilt*0.05;
			  ...
			  dZ1=dZ2; dZ2=SIN(ele)*step; newZ=tubeZ+dZ2;
  ...
  for ( __int64 z=cZ-rad/shape; z < cZ+rad/shape; z++ )
	for ( __int64 y=cY-rad; y < cY+rad; y++ )
	  for ( __int64 x=cX-rad; x < cX+rad; x++ )
		{ dX=(x-cX); dY=y-cY; dZ=(z-cZ)*shape;  dY=(y-cY)*shape;
		  dist=sqrt(dX*dX+dY*dY+dZ*dZ);
          if ( dist > rad ) continue;
		  if ( z < botGapVox+4 ) continue;  // flat top and bottom
		  if ( z > botGapVox+cellZvox-5 ) continue;
		  if ( dist <= rad )       matrix[x+(y*matrixX)+(z*matrixXY)]=memb;
		  if ( dist <= rad-thick ) matrix[x+(y*matrixX)+(z*matrixXY)]=lumen;
		  if ( z < botGapVox+5 )   matrix[x+(y*matrixX)+(z*matrixXY)]=memb;
		  if ( z > botGapVox+cellZvox-6 ) matrix[x+(y*matrixX)+(z*matrixXY)]=memb;
		}
...
{ __int64 left=sideGapVox,right=matrixX-sideGapVox;
  ...
  if ( tubes )  // set Matrix for elongated vesicles
	{ int rad=tubeRadVox,dist=interTubeDistNm/nmVox,tubeZ=tubeZnm/nmVox;
	  int axisY=up+dist/2,gap=poreRadNm/nmVox;
	  float cZ=0.0;
	  for ( vesNum=0; axisY <= down-rad; axisY+=dist,vesNum++ )
		{ for ( __int64 x=tubeLeftVox; x <= tubeRightVox; x++ )
			{ cZ=(x-tubeLeftVox)*bendTilt;
			  FillSphere(rad,x,axisY,tubeZ+cZ,VESI_MEMB,V_LUMEN,CELL_MEMB);
			}
		  if ( mitochondria ) // internal tube
			for ( __int64 x=tubeLeftVox; x <= tubeRightVox; x++ )
			  { cZ=(x-tubeLeftVox)*bendTilt;
				FillSphere(rad-5,x,axisY,tubeZ+cZ,TUBE_MEMB,T_LUMEN,VESI_MEMB);
			  }
	  	  if ( mitochondria )
			for ( __int64 x=tubeLeftVox; x <= tubeRightVox; x+=gap*4 )
			  { cZ=(x-tubeLeftVox)*bendTilt;
		  ...
		  vesicle[vesNum].x=tubeLeftVox*nmVox;vesicle[vesNum].y=axisY*nmVox;
		  vesicle[vesNum].z=(tubeZ)*nmVox;    vesicle[vesNum].xVox=tubeLeftVox;
		  vesicle[vesNum].yVox=axisY;         vesicle[vesNum].zVox=(tubeZ);
		  vesicle[vesNum].radNm=tubeRadNm;
		}
	}
	...
	{ int rad=vesRadNm/nmVox,dist,medium=CYTOPLASM,num=0;  bool noGo=false;
	  ...
	  for ( int i=0,vesXnm,vesYnm,vesZnm; i < vesNum; i++,num=i,noGo=false )
		{ for ( int t=0; t < 100; t++ )  /// try to find free space 100 times
		  ...
		  if ( noGo ) break; //
	 /*  vesYnm=500+sideGapNm;  // For illustartion purpose only
	   ...
	   */
		  ...
		  vesicle[i].docked=vesicle[i].fused=false;
		  FillSphere(rad,vesicle[i].xVox,vesicle[i].yVox,vesicle[i].zVox,VESI_MEMB,V_LUMEN,VESI_MEMB);
		 }
...
{ Thickening->Enabled=verticesReady=membOK=fuseTubes=false;Canvas->Font->Size=20;
  SetMembVertices->Enabled=false; MembFromVertices->Enabled=false;
  note="Making Geometrical Shapes => WAIT";
  Canvas->TextOut(20,20,"Making Geometrical Shapes => WAIT      ");
  Cursor=crHourGlass; tubeStage=0; shrinkLen=tubeLenVox;
  SetMembVertices->Enabled=membOK=MakeBody();  fissionCoef=0.0;
...
{ ProgBar->Visible=true; byte voxVal;  Canvas->Font->Size=20;
  Canvas->TextOut(20,20,"Thickening Membranes => WAIT      ");
  ...
  Canvas->TextOut(20,20,"Thickening Membranes => DONE     ");
  ProgBar->Visible=false; FormPaint(Sender); Canvas->Font->Size=8;
}
//---------------------------------------------------------------------------
...
{ int xx=2000,yy=2000,zz=0,mType=0,vShift=vesRadNm;
  __int64 x,y,z;
  if ( tubes ) vShift=tubeRadNm; // place memb object in a tube wall
  // cytoA init ------------------------------------------------------
  ...
  if ( membAinit == 4 ) ObjToVes(MembA,membAnum,vShift); // vesicle membrane
  for ( int i=0; i < membAnum; i++ )
	{ if ( membAinit == 0 )
  ...
  if ( membBinit == 4 ) ObjToVes(MembB,membBnum,vShift); // vesicle membrane
  if ( membBinit == 0 )
  ...
  if ( membCinit == 4 ) ObjToVes(MembC,membCnum,vShift); // vesicle membrane
  if ( membCinit == 0 )
  ...
  // Static A init
  double azi,ele,curX=0,curY=0,curZ=100,pitch=statApitchNm,p,ang=0.0,hexRad=10.0;
  double aziH[6],eleH[6]; __int64 x64,y64,z64; int fN=0;
  for ( int i=0; i < 6; i++ )
	{ Cart2sph(0.0,COS(ang),SIN(ang),&p,&aziH[i],&eleH[i]); ang+=60.0; }
  ...
  // 4 - 6 core bundle in filopodia, 5 - all from centrosome, 6 -nucleopore
  // 7 - filament along edge
  if ( statAinit != 4 ) // all except bundle in filopodia
	for ( int i=0; i < statAnum; )
	  { for ( int j=0; j < 10100; j++ ) // seed first object in filament
		  { x64=random(matrixX); y64=random(matrixY); z64=100.0/nmVox+botGapVox;
			if ( statAinit == 5 ) // centrosome
			  { x64=matrixX*0.5;  y64=matrixY*0.5; z64=botGapVox+cellZvox*0.5; }
		...
		for ( int j=0; j < monoPerFilo; j++ ) // init 0 Strait Fibres in cytoplasm z=100nm
		  { if ( i >= statAnum ) break;  // all StatA done
			...
			if ( statAinit == 1 || statAinit == 3 || statAinit == 5 )
			  curZ+=pitch*SIN(ele); // 3D
			if ( statAinit == 2 || statAinit == 3 || statAinit == 5 )
			  azi+=RandOne()*bendTilt; // curved
			if ( statAinit == 3 || statAinit == 5 ) ele+=RandOne()*bendTilt;  // 3D-curved
	...
	for ( int i=0,j=0,con=0; i < statAnum; j++ )
	  { if ( filo[fN].x[j] <= 0.0 )
		...
		for ( int a=0; a < 6 && i < statAnum; a++,i++ )
		  { azi=filo[fN].azi[j]; ele=filo[fN].ele[j];
			StatA[i].x=filo[fN].x[j]+COS(ele+eleH[a])*COS(azi+aziH[a])*hexRad;
			StatA[i].y=filo[fN].y[j]+COS(ele+eleH[a])*SIN(azi+aziH[a])*hexRad;
			StatA[i].z=filo[fN].z[j]+SIN(ele+eleH[a])*hexRad;
		  }
	  }
   if ( statAinit == 6 ) // SPECIAL nucleopore
	 for ( int i=0,att=0; i < statAnum && att < statAnum; att++ )
	   { x64=random(matrixX); y64=random(matrixY); z64=botGapVox+4;
		 if ( matrix[x64+(y64*matrixX)+(z64*matrixXY)] != NUCL_MEMB ) continue;
		 curX=x64*nmVox;  curY=y64*nmVox; curZ=z64*nmVox; ang=random(360);
		 for ( int a=0; a < 8 && i < statAnum; a++,i++ )
		   { ang+=45; StatA[i].x=curX+SIN(ang)*poreRadNm;
			 StatA[i].y=curY+COS(ang)*poreRadNm; StatA[i].z=curZ;
		   }
	   }
   curX=sideGapNm+cellXnm-tubeRadNm-5; curY=sideGapNm+tubeRadNm+5;
   curZ=botGapNm+tubeRadNm+5;
   if ( roundedCellBody ) { curX-=bodyFilletNm; curY+=bodyFilletNm; } // move
   if ( statAinit == 7 ) // SPECIAL filament along the edge
	 for ( int i=0; i < statAnum && curY < sideGapNm+cellYnm-5; curY+=pitch, i++ )
	   { StatA[i].x=curX; StatA[i].y=curY; StatA[i].z=curZ; } // filament at right
...
{ float jump=JumpSizeNm(tmStep,cytoAmob),nX,nY,nZ,dX,dY,dZ,rXY,tX,tY,tZ,prob;
  ...
  double recycleRate=(1.0/(cytoArecycleRate*tmStep));
  __int64 xVox,yVox,zVox;  // 64 bit integer variables
  ...
  for ( int i=0,ind; i < cytoAnum; i++ )
	{ if ( CytoA[i].medium == NUCLEOPLASM || CytoA[i].medium == T_LUMEN || CytoA[i].medium == V_LUMEN )
	  ...
	  if ( cytoApoli /*&& CytoA[i].statAind > -1 */ && CytoA[i].cytoAind < 0 ) // seed bound
		if ( random(1000000) == 0 ) // seeding of bound CytoA
  ...
  if ( cytoAinit == 2 && recycling ) // return released MembA molecules to ER
	for ( __int64 i=0,x,y,z,xx,yy,zz,fail; i < cytoAnum; i++,fail=0 )
	  { if ( CytoA[i].medium != EXTRACELL ) continue; // not released
		if ( random(recycleRate+1.0) < 1 )
		  { for ( int j=0; j < 10010; j++ ) // try random XYZ position
			  { xx=random(matrixXnm);yy=random(matrixYnm);zz=random(matrixZnm);
				x=xx/nmVox,y=yy/nmVox,z=zz/nmVox;
				if ( matrix[x+y*matrixX+z*matrixXY] == T_LUMEN ) // right medium
				  { CytoA[i].x=xx; CytoA[i].y=yy; CytoA[i].z=zz; break; }
				if ( j > 9999 ) { fail=1; break; }
			  }
			if ( fail ) break; // abort assigement
			CytoA[i].azi=CytoA[i].ele=0.0;CytoA[i].vesicleInd=-1;CytoA[i].medium=T_LUMEN;
			for ( int j=0; j < 4; j++ ) CytoA[i].green[j]=CytoA[i].red[j]=false;
			for ( int j=0; j < greenCytoAnum; j++ )
			  if ( random(100) < greenCytoAfrac ) CytoA[i].green[j]=true;
			for ( int j=0; j < redCytoAnum; j++ )
			  if ( random(100) < redCytoAfrac ) CytoA[i].red[j]=true;
		  }
	  } // END of recycling
...
{ float dX,dY,dZ,prob;  int medium=CYTOPLASM;
  ...
  double recycleRate=(1.0/(membArecycleRate*tmStep));
  ...
  if ( membAinit == 3 && recycling ) // return released MembA molecules to ER
	for ( __int64 i=0,x,y,z,xx,yy,zz,fail; i < membAnum; i++,fail=0 )
	  { if ( MembA[i].vesicleInd > -1 ) continue; // still in vesicle
		if ( random(recycleRate+1.0) < 1 )
		  { for ( int j=0; j < 10010; j++ ) // random positions at mType voxels
			  { xx=random(matrixXnm);yy=random(matrixYnm);zz=random(matrixZnm);
				x=xx/nmVox,y=yy/nmVox,z=zz/nmVox;
...
{ float dX,dY; int moveRange=20;
  ...
  if ( Fission->Checked && tubes )
	{ Caption=Round(fissionCoef,3);
	  if ( fissionCoef < 1.0 )
		{ fissionCoef+=0.0001;
		  for ( int i=0; i < vesNum; i++ )
			TubeFission(tubeRadVox,vesicle[i].xVox,vesicle[i].yVox,vesicle[i].zVox,fissionCoef);
          for ( int i=0; i < cytoAnum; i++ ) // move
			MoveToNearestVoxel(&CytoA[i],20,CytoA[i].medium);
		  for ( int i=0; i < cytoBnum; i++ ) // move
			MoveToNearestVoxel(&CytoB[i],20,CytoB[i].medium);
		  for ( int i=0; i < membAnum; i++ ) // move
			MoveToNearestVoxel(&MembA[i],20,MembA[i].medium);
		  for ( int i=0; i < membBnum; i++ ) // move
			MoveToNearestVoxel(&MembB[i],20,MembB[i].medium);
		  for ( int i=0; i < membCnum; i++ ) // move
			MoveToNearestVoxel(&MembC[i],20,MembC[i].medium);
	   }
	}
  if ( fuseTubes )
	{ if ( tubeStage == 0 )
		{ //for ( int i=0; i < vesNum; i++ ) TubeFusion(i,tubeStage);
		  tubeStage=1;  // done pore
		  for ( int i=0; i < vesNum; i++ ) TubeFusion(i,tubeStage);
		  if ( cytoAinit == 3 )
			for ( int i=0; i < cytoAnum; i++ ) CytoA[i].medium=EXTRACELL;
		  if ( cytoBinit == 3 )
			for ( int i=0; i < cytoBnum; i++ ) CytoB[i].medium=EXTRACELL;
		  if ( membAinit == 4 )
			for ( int i=0; i < membAnum; i++ ) MembA[i].medium=CELL_MEMB;
		  if ( membBinit == 4 )
			for ( int i=0; i < membBnum; i++ ) MembB[i].medium=CELL_MEMB;
		  if ( membCinit == 4 )
			for ( int i=0; i < membBnum; i++ ) MembB[i].medium=CELL_MEMB;
		}
	  if ( tubeStage == 1 )
		{ for ( int i=0; i < vesNum; i++ ) TubeFusion(i,tubeStage);
		  shrinkLen--; // shorten the tubes
		  if ( shrinkLen < 1 ) { tubeStage=2; moveRange=tubeRadVox*3; }
          if ( shrinkLen < 1 && !transfer ) tubeStage=3;
		}
	   if ( tubeStage == 2 )
		{ for ( int i=0; i < vesNum; i++ ) TubeFusion(i,tubeStage); }
	  if ( tubeStage == 1 || tubeStage == 2 )
		{ for ( int i=0; i < cytoAnum; i++ ) // move
			MoveToNearestVoxel(&CytoA[i],moveRange,CytoA[i].medium);
		  for ( int i=0; i < cytoBnum; i++ ) // move
			MoveToNearestVoxel(&CytoB[i],moveRange,CytoB[i].medium);
		  for ( int i=0; i < membAnum; i++ ) // move
			MoveToNearestVoxel(&MembA[i],moveRange,MembA[i].medium);
		  for ( int i=0; i < membBnum; i++ ) // move
			MoveToNearestVoxel(&MembB[i],moveRange,MembB[i].medium);
		  for ( int i=0; i < membCnum; i++ ) // move
			MoveToNearestVoxel(&MembC[i],moveRange,MembC[i].medium);
		}
	}
  if ( movingVes )
	{ if ( recycling )
	  ...
	  if ( frameCount < 10000 ) // record spots
		for ( int i=0,n=frameCount; i < spotNum; i++ )
		  { spot[i].cX[n]=MembA[i].x*0.001;spot[i].cY[n]=MembA[i].y*0.001; // in µm
			MembA[i].z*0.001; spot[i].spInt[n]=random(20)*random(20)+20;
		  }
...
{ double dX=x-((cellXnm*0.5)+sideGapNm),dY=y-((cellYnm*0.5)+sideGapNm),gaussX,gaussY;
  if ( sqrt((dX*dX)+(dY*dY)) > irisRadUm*1000.0 ) return 0;
  ...
  if ( Confocal->Checked ) photons*=1.0/(1.0+(AbsD(z-focus)*0.003));
  ...
  int redShift=0,midPoint=sizeXpix*0.5;
  if ( twoCam && colour == RED ) redShift=midPoint;
  for ( int i=0,cX,cY; i < photons; i++ )
	{ gaussX=GaussRand()*sigma; gaussY=GaussRand()*sigma;
	  if ( Confocal->Checked )
		if ( AbsD((gaussX*gaussX+gaussY*gaussY)) > 20000 ) continue;
	  cX=(x+gaussX)/nmPix+redShift;
	  cY=(y+gaussY)/nmPix;
	  if ( twoCam && colour == RED && cX < midPoint ) continue; // green filter
	  if ( twoCam && colour == GREEN && cX > midPoint ) continue; // red filter
	  if ( cX < 0 || cX > sizeXpix-1 || cY < 0 || cY > sizeYpix-1 ) continue;
	  if ( cell[cX][cY] < 64000 ) cell[cX][cY]++;
	}
  return photons;
}
//---------------------------------------------------------------------------
	...
	{ if ( blueLaser ) // Green fluorescence
		for ( int j=0; j < 4; j++ )
		  { //if ( obj[i].cytoBind < 0 ) continue; // for calcium imaging only
			...
			if ( greenBlinking )  // only blinking mode
...
{ static int indAA=0; double averAA=0.0;
  ...
  if ( saveFrame ) { str+="  (";str+=savedFrames;str+=" saved Frames)"; }
...
{ // Cell Properties
  ...
  SForm->cond.rafts=rafts;  SForm->cond.tubeZnm=tubeZnm; SForm->cond.raftNum=raftNum;
  ...
  SForm->GreenBlinkingChBox->Checked=greenBlinking;
...
{  if ( SForm->cond.memorySizeMb != memorySizeMb )  // resize main memory
  ...
  tubeZnm=SForm->cond.tubeZnm; raftNum=SForm->cond.raftNum;
  // imaging prop
...
{ if ( nmPix < 10 ) return;
  ...
  if ( CCDwidth > 2540 || CCDheight > 2040 )
...
{  TIRillum->Checked=true; StepCalc(); Illum->Caption="Mode(TIRFM)"; FormPaint(Sender);}
//---------------------------------------------------------------------------
void __fastcall TMainForm::EpiIllumClick(TObject *Sender)
{ EpiIllum->Checked=true; StepCalc(); Illum->Caption="Mode(Epi)"; FormPaint(Sender); }
//---------------------------------------------------------------------------
void __fastcall TMainForm::ConfocalClick(TObject *Sender)
{ Confocal->Checked=true; StepCalc(); Illum->Caption="Mode(Conf)"; FormPaint(Sender); }
//---------------------------------------------------------------------------
...
{ int max=0,szX=matrixXnm/nmPix,szY=(matrixYnm)/nmPix;
  ...
  savedFrames++;
  addPrm.int1=membAboundMembA; addPrm.float3=membAunitsInPoli;
  addPrm.int2=cytoAboundToMembAnum;
...
{ if ( !SaveDlg->Execute() ) return;
  ...
  savedFrames=1; frameCount=cycleNum=0;time=0.0;saveFrame=true;
  Caption="GMvCell-"+ SaveDlg->FileName;
  RunClick(Sender);
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void __fastcall TMainForm::BleachObj(Obj *obj,int num)
{ int centX=cellXnm/2+sideGapNm,centY=sideGapNm+cellYnm/2,dX,dY;
  int boader=sideGapNm+cellXnm-bleachSizeNm,rad=bleachSizeNm/2,zMemb=botGapNm+5*nmVox;
  bool bleachGreen=false,bleachRed=false;
  for ( int i=0; i < num; i++ )
	if ( obj[i].z < zMemb || EpiIllum->Checked || Confocal->Checked ) // zMemb for TIRFM
	  { dX=obj[i].x-centX; dY=obj[i].y-centY;
		...
		if ( bleachStrip && MembA[i].x > boader )
		  { bleachGreen=blueLaser;  bleachRed=GreenLaser; }
		if ( bleachGreen ) for ( int j=0; j < 4; j++ ) obj[i].green[j]=false;
		if ( bleachRed )   for ( int j=0; j < 4; j++ ) obj[i].red[j]=false;
		bleachRed=bleachGreen=false;
	  }
}
//---------------------------------------------------------------------------
void  TMainForm::Bleaching(void)
{ BleachObj(CytoA,cytoAnum); BleachObj(CytoB,cytoBnum);
  BleachObj(MembA,membAnum); BleachObj(MembB,membBnum); BleachObj(MembC,membCnum);
  BleachObj(StatA,statAnum);
...
{ ReadSettings(1); CalculateDimensions();
  TrackBar->Max=matrixZnm;TrackBar->PageSize=cellZnm/10;TrackBar->Position=focusNm;
...
{ blueLaser=!blueLaser;
  if ( !twoCam && greenLaser ) { GreenLaser->Caption="GreenLaser_OFF"; greenLaser=0; }
...
{ greenLaser=!greenLaser;
  if ( !twoCam && blueLaser ) { BlueLaser->Caption="BlueLaser_OFF"; blueLaser=0; }
  if ( greenLaser ) GreenLaser->Caption="GreenLaser_ON";
...
{ if ( !SaveLogDlg->Execute() ) return;
  ...
  for ( int num=0; num < frameCount && num < 10000; num++ )
...
//---------------------------------------------------------------------------
void __fastcall TMainForm::SaveGMIClick(TObject *Sender) // use addInfo=8 for xyz
{ FILE *outfile; float buf=0.0; int intBuf=2022,addInfo=0; // last format 2022
  ...
  for ( int num=0; num < spotNum; num++ )
	 { fwrite(&intBuf,4,1,outfile); // new format 2022
	   ...
	   fwrite(&tm[1],4*spot[num].len,1,outfile); // time (seconds)
...
//---------------------------------------------------------------------------
void __fastcall TMainForm::FuseTubesClick(TObject *Sender)
{ fuseTubes=true; }
//---------------------------------------------------------------------------
